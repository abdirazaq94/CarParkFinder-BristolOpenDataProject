...
The car park finder is a web application made using HTML, CSS and JavaScript by using Bristols open data api to find car park locations in the City of Bristol. The application itself is made up of a home page, a list view and an interactive map view. All logic runs client side with no backend, server or external JavaScript libraries besides the maps use of Leaflet.js.
...
Fields taken for use: NAME, NUMBER_LEVELS, AREA_NAME, OPERATOR_NAME, SPACES, OPERATING_TIMES, TYPE_DESCRIPTION, CCTV
...
- The map view uses Leaflet.js retrieved from a CDN (unpkg.com) and requires an internet connection.
...
|---|---|---|
| index.html | 14 | Unexpected 'for', Unexpected 'let', Line longer than 80 characters (x7), Unexpected trailing space (x4) |
| map.html | 16 | Unexpected 'for', Unexpected 'let', Use double quotes not single quotes (x3), Line longer than 80 characters (x3), Unexpected trailing space (x7), Expected ';' saw 'let', Expected property 'attribution' ordered before 'maxZoom' |

The warnings in both modules are predominantly stylistic — trailing whitespace from indentation, lines exceeding 80 characters due to the length of the API URL, and JSLints preference for while loops over for loops with let. These do not affect the functionality of the application. The single/double quote warnings in map.html relate to Leaflet.js configuration strings. No logical or functional errors were reported.
...
The web application uses a three-page client-side architecture. All pages share the same stylesheet. index.html and map.html separately fetch from the same Bristol Open Data API endpoint.
...
2. Read the introduction to the application.
3. Click Find Your Car Park to go to the list view.

### List View (index.html)

1. Car park data loads automatically — Loading list... appears while fetching.
2. All Bristol car parks are shown in the results table once loaded.
3. Search: Type a name or area in the search box and click Search.
4. Filter: Tick any combination of Multi Storey, Surface, Underground, or Camera — results update instantly.
5. Map View: Click Map View in the left sidebar to switch to the map.
6. Reset: Click View All Results to clear search and filters and show all car parks.

### Map View (map.html)

1. Car parks load automatically as pins on an OpenStreetMap centred on Bristol.
2. Click any pin to see the car park name in a popup.
3. Search/Filter: Use the same controls — type a search term or tick filters and click Search to show/hide pins.
4. List View: Click List View in the left sidebar to switch back.
5. Reset: Click View All Results to reset and show all pins.
